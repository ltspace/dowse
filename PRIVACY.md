...
- The MCP server (`dowse mcp`) is **read-only**: it exposes search, preview, bounded indexed-content reading, and index-status tools over stdio to the AI client you attach it to. It cannot modify your files or the index, and the content-reading tool only returns text already stored in the index rather than opening arbitrary paths.
...
The dowse MCP server makes no network requests. All indexing, searching, OCR, preview, and indexed-content reading operations run entirely on your machine.
