...
| 指标 | 目标 | 实测（第二轮，2026-07-12） | 说明 |
|------|------|------|------|
| 快捷键到窗口可见 | < 50ms | 本轮未测 | 本轮为纯 CLI 基准，未接入浮窗应用埋点 |
| 键入到结果上屏 | < 80ms | 本轮未测 | 同上 |
| OCR | ~112ms / 1080p 截图 | ~100–125ms / 张 | 100 张合成的 480×200 中英文渲染图，由 `dowse index` 的 OCR 队列批量跑完；不是真实 1080p 截图的测试 |
| 常驻内存 | < 150MB | 本轮未测（见下方索引期间峰值） | 该目标是浮窗应用空闲态内存，本轮 CLI 基准只测了索引过程中的峰值 working set |
| 安装包 | < 15MB | 本轮未测 | 本轮基准不含打包步骤 |
| 文件名索引构建（规划） | 秒级 | 10,000 文件 / 437MB 语料，全文内容索引耗时 27–34s | 不是规划中的纯文件名 MFT 直读，是当前 `dowse index` 的全文内容索引 |
| 全量索引吞吐（新增） | — | ~13–16MB/s，~300–375 文件/s | 同上一轮基准用的 10k 文件/437MB 语料，上一轮吞吐约 47MB/s |
| 搜索延迟 P50（新增） | — | 常见词/哨兵词/短语查询普遍落在 ~250–450ms | CLI 每次查询单独起进程，含启动开销；相比上一轮，短语查询延迟大幅下降，简单查询延迟上升 |
| 索引体积 ÷ 语料体积（新增） | — | ~0.64–0.66 | 上一轮基准约 0.76 |

全量索引相关行基于 10,000 文件 / 437MB 的合成中英文混排语料实测（i7-13700K，24 逻辑核，64GB 内存），单机单次会话。
