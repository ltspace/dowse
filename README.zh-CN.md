...
设计目标，超出即视为缺陷。"实测"列是第三轮从零重跑的基准测试（`dowse 0.6.1`，
i7-13700K / 24 逻辑核 / 64GB 内存，单机单次会话，2026-07-12）。完整方法论与原始数据
（`BENCH-REPORT-V3.md`）留在本次基准的工作目录里，不进本仓库。

| 指标 | 设计目标 | 实测（第三轮，v0.6.1，2026-07-12） |
|---|---|---|
| 快捷键到窗口可见 | < 50ms | 本轮未测——纯 CLI 基准，未接入浮窗应用埋点 |
| 键入到结果上屏 | < 80ms | 本轮未测——同上 |
| OCR 单张 | ~112ms / 1080p 截图 | 单张孤立测约 176ms；批量跑满队列吞吐约 12–19ms/张（工作线程池+批量提交摊薄）——测试图是合成的 480×200 图片，不是真实 1080p 截图 |
| 常驻内存 | < 150MB（空闲态） | 空闲态未测；全量索引过程中的峰值 working set 约 507MB——这是另一个指标，不能直接对照空闲态目标判定回归 |
| 安装包体积 | < 15MB | **9.75MB**（`dowse-app_0.6.1_x64-setup.exe`，已发布版本） |
| 全文索引构建，10,000 文件 / 437MB | 秒级（规划中的纯文件名快速路径） | 19.8–21.4s——当前的全文内容 `dowse index`，不是规划中的纯文件名 MFT 路径 |
| 全文索引构建 + OCR，15,100 文件（含 5,100 张图） | — | 首次约 83s（5,015/5,100 张图完成 OCR；写入冲突重试预算耗尽剩 85 张，第二次运行 2s 内补完——可续跑，不丢失） |
| 搜索延迟 P50（任务要求的 5 类） | — | 单词/中文短语/英文短语/多词 AND/无结果五类落在 149–178ms，基于 15,100 篇文档的索引 |
| 搜索延迟 P95 | — | 同五类，155–186ms |
| `ext:` 过滤查询延迟 | — | P50 153ms，与不带过滤的查询同一量级 |
| 索引体积 ÷ 语料体积 | — | 0.54（纯文本），低于上一轮基准的 0.64–0.66，也低于两轮前的 0.76 |

全量索引相关行基于 10,000 文件 / 437.66MB 的文本语料，加上 5,100 张合成 480×200 OCR 测试图
（89.8MB）——文本语料体积特意与前两轮基准对齐，方便对比。前两轮全量重建已有完整记录：
上一轮发现全量索引耗时约变慢 3 倍、连续快速重建的崩溃率约 95%；这一轮两个问题都没有复现——
索引耗时回到接近第一轮的水平，本轮遇到的每一次瞬时写入冲突都被自动重试机制接住，没有一次
不可恢复的失败。
...
如今我在一家做 AI 搜索基础设施的公司工作，用 MCP 的方式让 AI 检索网络上的数据。科幻电影《她》像一则预言，不远的将来，AI 会接管我们的个人电脑。dowse 受此启发，也提供 MCP 接口供 AI 调用，只不过检索的是你自己的文件，在你自己的机器上。
